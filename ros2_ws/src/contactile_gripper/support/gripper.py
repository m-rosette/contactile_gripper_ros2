...
import support.dynamixel_motors as dynamixel_motors
import time
import atexit
...
class Gripper(Node):
    ...
    def calibrate(self):
        ...
        self.motor.node.get_logger().info('[CALIBRATE GRIPPER START]')
        ...
        self.motor.node.get_logger().info('Gripper range: {}'.format(self.grip_range))
        self.motor.node.get_logger().info('[CALIBRATE GRIPPER COMPLETE]')
    ...
    def _calibrate_close(self):
        ...
        while True:
            ...
            if vel > max_velocity:
                self.motor.write_goal_cur(closing_current // 3)
            else:
                self.motor.write_goal_cur(closing_current)
        self.motor.node.get_logger().info('[MAX_POS_FULLY_CLOSED] {}'.format(self.motor.MAX_POS_FULLY_CLOSED))
    ...
    def fully_open(self):
        ...
        self.motor.node.get_logger().info('[MIN_POS_FULLY_OPEN] {}'.format(self.motor.MIN_POS_FULLY_OPEN))
    ...
    def switch_modes(self, mode):
        ...
        torque is on."""
        self.motor.node.get_logger().debug('[switch_modes] {}'.format(mode))
