...
class Camera:
    ...
    def __find_aruco(self):
        ...
        self.corners, self.ids, rejectedImgPoints = aruco.detectMarkers(
    ...
    def __aruco_transform(self):
        ...
        rot_mat = cv2.getRotationMatrix2D(
            tuple(self.aruco_center), aruco_ang, 1.0)
        self.frame = cv2.warpAffine(
            self.frame, rot_mat, self.frame.shape[1::-1], flags=cv2.INTER_LINEAR)
    ...
    def __find_cable_ang(self):
        ...
        self.cable_slope = self.__find_slope(
            self.color_centers[0], self.color_centers[1])
    ...
    def __find_cable_pos(self):
        ...
        cable_b = self.__find_y_intercept(
            self.cable_slope, self.color_centers[0])
    ...
    def __calibrate_cam(self, test=False, see_corners=False):
        ...
        for filename in images:
            ...
            image_good, corners = cv2.findChessboardCorners(
                grayColor, checkerboard_corners, cv2.CALIB_CB_ADAPTIVE_THRESH + cv2.CALIB_CB_FAST_CHECK + cv2.CALIB_CB_NORMALIZE_IMAGE)
            if image_good:
            ...
            if image_good and see_corners:
                ...
                image = cv2.drawChessboardCorners(
                    image, checkerboard_corners, corners2, image_good)
        ...
        ret, self.mtx, self.dist, r_vecs, t_vecs = cv2.calibrateCamera(
    ...
    def __add_cable_illustration(self):
        ...
        cv2.line(self.frame, tuple(self.color_centers[0]), tuple(
            self.color_centers[1]), color=(0, 0, 255), thickness=1)
    ...
    def __find_dev_path(self):
        ...
        return f'/dev/video{vid_device_num[0]}'
